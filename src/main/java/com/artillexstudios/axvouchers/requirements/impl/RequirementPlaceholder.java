...
public class RequirementPlaceholder extends Requirement {
    ...
    public boolean check(Player player, Voucher voucher, String arguments) {
        if (!ClassUtils.INSTANCE.classExists("me.clip.placeholderapi.PlaceholderAPI")) {
            log.warn("PlaceholderAPI is not present on this server, yet you tried to use placeholder requirements in voucher {}!", voucher.getId());
            return false;
        }
        ...
        if (arguments.contains("|")) {
            ...
            sendFail(player, Placeholder.parsed("world", String.join(Messages.OR, split)));
            return false;
            ...
            for (String s : split) {
                ...
                String value = sides[1];
                if (!parsed.equalsIgnoreCase(value)) {
                    sendFail(player, Placeholder.parsed("world", value));
                    return false;
                }
            }
    ...
    public void sendFail(Player player, TagResolver... resolvers) {
        if (Config.sendRequirementFail) {
            player.sendMessage(StringUtils.formatToString(Messages.PREFIX + Messages.FAIL_PLACEHOLDER, resolvers));
        }
    }
}
